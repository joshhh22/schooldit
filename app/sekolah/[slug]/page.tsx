...
export default function SchoolDetailPage() {
    ...
    return (
      <div className="reddit-card p-10 text-center space-y-4 bg-white dark:bg-[#0f1626] border-slate-200 dark:border-[#1e293b] font-sans">
        <h2 className="font-heading text-base font-bold text-slate-900 dark:text-white">
          Komunitas Sekolah Tidak Ditemukan
        </h2>
  ...
  const schoolPosts = posts.filter(
    (p) => p.schoolId === school.id || p.schoolSlug === school.slug
  );
        ...
        <div className="p-4 sm:p-5 -mt-8">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            >
              ...
              <span>Create Post in s/{school.slug}</span>
            </Link>
          </div>
          ...
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-100 dark:border-[#1e293b] text-[11px] text-slate-400">
            <span className="flex items-center gap-1">
              ...
              <strong className="text-slate-800 dark:text-slate-200">{school.memberCount}</strong> siswa
            </span>
            <span className="flex items-center gap-1">
              <MessageSquare className="w-3.5 h-3.5" />
              <strong className="text-slate-800 dark:text-slate-200">{schoolPosts.length}</strong> postingan
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {schoolPosts.length > 0 ? (
          ...
          <div className="reddit-card p-10 text-center space-y-3 bg-white dark:bg-[#0f1626] border-slate-200 dark:border-[#1e293b]">
            <SchoolIcon className="w-8 h-8 text-slate-400 mx-auto" />
            <h3 className="font-heading text-sm font-bold text-slate-800 dark:text-slate-200">
              Belum Ada Postingan di s/{school.slug}
            </h3>
            <p className="text-xs text-slate-400">
              Jadilah yang pertama membuat postingan atau spill obrolan di komunitas ini!
            </p>
            <div className="pt-2">
              <Link
                href={`/posting?school=${school.id}`}
